...
void FaustDspUi::PushModel(Model& model) {
...
void FaustDspUi::addVerticalSlider(const char* label, float* zone, float init, float min, float max, float step) {
...
void FaustDspUi::addHorizontalSlider(const char* label, float* zone, float init, float min, float max, float step) {
  if(zones_[zone].isKnob()) {
    return addKnob(label, zone, init, min, max, step);
  }
...
void FaustDspUi::addKnob(const char* label, float* zone, float init, float min, float max, float step) {
  AddModel(*new Knob(label, zone, init, min, max, step));
}

void FaustDspUi::addNumEntry(const char* label, float* zone, float init, float min, float max, float step) {
...
void FaustDspUi::addTextDisplay(const char* label, float* zone, char* names[], float min, float max) {
  AddModel(*new TextDisplay(label, zone, names, min, max));
}

void FaustDspUi::addHorizontalBargraph(const char* label, float* zone, float min, float max) {
  AddModel(*new HBarGraph(label, zone, min, max));
}

void FaustDspUi::addVerticalBargraph(const char* label, float* zone, float min, float max) {
